<template>
  <a-card>
    <a-button v-if="$hasp('monitoring-manage')" class="management" @click="management">
      监控设备管理
    </a-button>
    <!-- 百度地图容器 -->
    ...
    <!-- 监控设备列表弹窗 -->
    <modal v-if="$hasp('monitoring-manage')" v-model="visible" title="监控设备管理" style="transform: translateX(0)" width="100%" top="110" :footer="null" @cancel="visible = false">
      <StandardTable :columns="columns" :data-source="dataSource" :loading="loading" :pagination.sync="params.page" :selected-rows.sync="selectedRows" @search="search" @change="pageChange">
        <template slot="btns">
          <a-button v-if="$hasp('monitoring-add')" type="primary" @click="add">
            新增
          </a-button>
          <a-button v-if="$hasp('monitoring-edit')" type="primary" @click="edit">
            修改
          </a-button>
          <a-popconfirm
            v-if="$hasp('monitoring-del')"
    ...
    <!-- 新增、修改监控设备弹窗 -->
    <modal v-if="$hasp('monitoring-add') || $hasp('monitoring-edit')" v-model="visible1" :title="title" width="400px" @ok="handleOk" @cancel="visible1 = false">
      <a-form-model ref="ruleForm" :model="dataForm" label-align="left">
        <a-row :gutter="[16, 0]">
          ...
          <a-col span="24">
            ...
            >
              <a-input v-model="dataForm.name" class="w-100" :max-length="100" autocomplete="off" allow-clear placeholder="请输入工设备名称" />
            </a-form-model-item>
          </a-col>
          <a-col span="24">
            <a-form-model-item
              ...
              prop="monitorNum"
            >
              <a-input v-model="dataForm.monitorNum" class="w-100" :max-length="100" autocomplete="off" allow-clear placeholder="请输入设备编号" />
            </a-form-model-item>
          </a-col>
          <a-col span="10">
            <a-form-model-item
    ...
    <!-- 视频播放弹窗 -->
    <shade v-if="$hasp('monitoring-query')" v-model="mask" bgc="transparent" height="650">
      <!-- 视频盒子 -->
      <div class="container">
        <!-- 当前设备信息 -->
    ...
    <!-- 选择坐标地图 -->
    <map-select v-if="$hasp('monitoring-add') || $hasp('monitoring-edit')" v-model="isSelect" @mapClick="mapClick" />
  </a-card>
</template>
...
import { getMonitorList, saveMonitroData, delMonitorData, getAccessToken, startControl, endControl } from '@/api/modules/monitoring'
...
export default {
  ...
  data() {
    return {
      ...
      columns: columns.map(item => item.title === '操作' ? ({ ...item, visible: this.$hasp(['monitoring-edit', 'monitoring-del']) }) : item), // 表头数据
      dataSource: [], // 表格数据
      selectedRows: this.$hasp('monitoring-edit') || this.$hasp('monitoring-del') ? [] : undefined, // 选择的数据
  ...
  methods: {
    ...
    edit() {
      ...
      this.title = '修改监控设备'
      this.dataForm = cloneDeep(this.selectedRows[0])
      this.visible1 = true
    },
    /**
     * 选择地理坐标
    ...
    handleOk() {
      ...
      this.$refs.ruleForm.validate(async val => {
        ...
        this.resetLabel()
        this.visible1 = false
        if (this.title === '新增监控设备') return this.$message.success('添加成功！')
